...
public class EquipedTransform // 장착된 장비 오브젝트(플레이어에서 On/Off로 관리), 장비ID
{
    public GameObject equipmentTransform;
...
public class UIInventory : MonoBehaviour
{
    ...
    private int curEquipmentId;
    public EquipedTransform[] equipedTransforms;
    ...
    void Start()
    {
        ...
        controller.onInventoryInput += ToggleInventoryUI;
    ...
    public void ToggleInventoryUI() // 켜져있다면 false, 꺼져있다면 true
    {
        if (IsOpen())
        {
            inventoryWindow.SetActive(false);
            PlayerManager.Instance.Player.playerMovement.canLook = true;
            PlayerManager.Instance.Player.playerController.SetCursor();
        }
        else
        {
            inventoryWindow.SetActive(true);
            PlayerManager.Instance.Player.playerMovement.canLook = false;
            PlayerManager.Instance.Player.playerController.SetCursor();
        }
    }
    ...
    {   //여기 itemData는 현재 상호작용된 아이템의 데이터를 넣어놨다.
        DefaultItem data = PlayerManager.Instance.Player.itemData;
    ...
    public void ThrowItem(DefaultItem data) // 아이템 떨어뜨리기
    ...
    ItemSlot GetItemStack(DefaultItem data)
    {
        for (int i = 0; i < slots.Length; i++) // 슬롯 배열 순회
    ...
    public void SelectItem(int index) // 우리가 고른 아이템, 슬롯번호만 받아온다.  => 입는건지 먹는건지 구별하는 방법..
    {
        ...
        selectedItemDescription.text = selectedItem.item.itemDescription; // 그리고 설명을 적어준다.
    ...
    public void OnUseButton() // 사용하기
    {
        ...
        {
            for (int i = 0; i < (selectedItem.item as ConsumableItem).consumableValuePerTypes.Length; i++) // 아이템 조회
            {
                switch ((selectedItem.item as ConsumableItem).consumableValuePerTypes[i].type) //i번째 type을 보고
                {   //우리는 29번째줄에 condition을 가져왔었으니
                    case ConsumableType.HP: // 체력 회복
                        condition.Heal((selectedItem.item as ConsumableItem).consumableValuePerTypes[i].value); break;
                    case ConsumableType.Hunger: // 배고픔 회복
                        condition.Eat((selectedItem.item as ConsumableItem).consumableValuePerTypes[i].value); break;
                    case ConsumableType.Thirsty: // 목마름 회복
                        condition.Drink((selectedItem.item as ConsumableItem).consumableValuePerTypes[i].value); break;
                    case ConsumableType.Speed:
                        // TODO : 일정시간 동안 플레이어 스피드 증가
                        break;
                    case ConsumableType.Damage:
                        // TODO : 일정시간 동안 플레이어 데미지 증가
                        break;
                }
            }
    ...
    public bool HasItem(DefaultItem item, int quantity)
    {
        return false;
    }
}